...
## Model 2
The goal of Model 2 is to create an accurate model to predict congestion speed by leveraging multiple features in the dataset such as weather, location, and time data.

XGBoost was selected due to its exceptional performance, scalability, and robustness on large datasets, as well as its capability to capture non-linear relationships, which enables generalization to unseen data. Based on the results of the XGBoost model we can see that as the number of boosting rounds increases the error rate decreases. However, eventually at around 400 boosting rounds, the error rate on the test set begins to stabilize at 35% which indicates a 65% accuracy. Furthermore, the most impactful features were location data (longitude, latitude) and start time. This may indicate that traffic congestion is highly dependant on location and time of day which lines up with our intuition. 
