...
Reflecting on our approach and outcomes, there are several key takeaways and areas for potential improvement.  One key takeaway from our research was the potential for using traffic and weather data to determine severity and delays in traffic conditions.  The dataset provided information on weather events, visibility, wind speed, and precipitation, as well as the time and location of congestion events and their impact on commute times. 
...
For future research, we could enhance our models by integrating more features, such as more detailed weather conditions, economic indicators, or even additional temporal and spatial factors. Looking for a more evenly distributed data set or processing everything better since a lot of the feature values were skewed because of times in the middle of the night (12am-2am).  Also running the research in a way where we could have higher runtimes as we couldn't do large computation due to it taking too long or crashing.

Also, exploring more higher quality data preprocessing techniques to handle skewed distributions if they do occur and using more balanced datasets could lead to better model performance. Addressing runtime limitations through optimized code and more powerful computational resources would enable us to handle larger datasets and perform more extensive analyses.

In conclusion, our research provided valuable insights into traffic congestion patterns and showed the potential of machine learning models to predict congestion severity and delays.  While we did run into a few speed bumps in our data distributions and runtime limitations, our findings lay a solid foundation for future research and model tuning.  By adding features, improving on data preprocessing, and using more advanced modeling techniques, we would be able to develop more accurate predictive models to begin addressing traffic congestion challenges.
