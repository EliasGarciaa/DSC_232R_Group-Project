...
## Model 2
Based on the results of the XGBoost model we can see that as the number of boosting rounds increases the error rate decreases. However, eventually at around 400 boosting rounds, the error rate on the test set begins to stabilize. Furthermore, the most impactful features were location data (longitude, latitude) and start time. This may indicate that traffic congestion is highly dependant on location and time of day which lines up with our intuition. 
